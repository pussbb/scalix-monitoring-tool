...
class BaseModel:
    ...
    @classmethod
    async def add(cls, engine: 'aiopg._EngineContextManager', **kwargs):
        async with engine.acquire() as conn:
            await conn.execute(cls.insert_query().values(**kwargs))
    ...
    @classmethod
    async def find(cls, engine: 'aiopg._EngineContextManager', id_: Any):
        query = cls.select_query().where(cls.table.c.id == id_)
        async with engine.acquire() as conn:
            item = await (await conn.execute(query)).first()
            if not item:
                raise RecordNotFount('by id {}'.format(id_))
            return cls(engine, **item)

    @staticmethod
    def time_range_str(time_filter):
        time_range = " t.ts > (NOW() - INTERVAL '1 day 20 minute')"
        if time_filter:
            time_range = " t.ts BETWEEN {0} AND {1}".format(*time_filter)
        return time_range
